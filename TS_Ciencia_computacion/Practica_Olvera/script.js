...
function Animacion(){
   ...
   this.naveConTeclas = null; // nave se mueve con las teclas
   ...
   */
      ...
      this.naveConTeclas.dibujar(this.auxcontexto); // Dibuja la nave en el auxiliar
   ...
   */
      ...
      this.naveConTeclas.sdireccion((this.naveConTeclas.gdireccion()) +  this.naveConTeclas.getVelocidad()); // Mueve la nave
      ...
      if((this.naveConTeclas.getX()>this.canvas.width - this.naveConTeclas.getImagen().width)){ // Si la nave sale por la derecha
         this.naveConTeclas.setX(this.canvas.width - this.naveConTeclas.getImagen().width);
         this.naveConTeclas.setVelocidad(this.naveConTeclas.getVelocidad()*-1);
      }

      if( (this.naveConTeclas.getX() <= 0)){ // Si la nave sale por la izquierda
         this.naveConTeclas.setX(0);
         this.naveConTeclas.setVelocidad(this.naveConTeclas.getVelocidad()*-1);
      }

      if( (this.naveConTeclas.getY()>=this.canvas.height - this.naveConTeclas.getImagen().height)){ // si la nave sale por la parte abajo
         this.naveConTeclas.setY(this.canvas.height - this.naveConTeclas.getImagen().height);
         this.naveConTeclas.setVelocidad(this.naveConTeclas.getVelocidad()*-1);
      }

      if( (this.naveConTeclas.getY()<=0)){ // si la nave sale por la parte arriba
         this.naveConTeclas.setY(0);
         this.naveConTeclas.setVelocidad(this.naveConTeclas.getVelocidad()*-1);
      }


      // La nave nodrisa se mueve aleatoriamente      
         setInterval(function(){ // Este intervalo se ejecuta cada 1 segundo
            var posicion = "" // Variable para guardar la posicion de la nave nodrisa
            var arreglo = [-1, 1,1,-1]; // Arreglo para guardar las posiciones de las naves
             for (i=0;i<arreglo.length;i++){ // Recorre el arreglo
               posicion = arreglo[Math.floor(Math.random()*arreglo.length)]; // Selecciona una posicion al azar
            }
            if(posicion == -1){ // Si la posicion es arriba
               objeto.naveNodrisa.gdireccion = objeto.naveNodrisa.getX; // Guarda la posicion de la nave nodrisa
               objeto.naveNodrisa.sdireccion = objeto.naveNodrisa.setX; 
            } else if(posicion == 1){ // Si la posicion es abajo
               objeto.naveNodrisa.gdireccion = objeto.naveNodrisa.getY;
               objeto.naveNodrisa.sdireccion = objeto.naveNodrisa.setY;
            } else if(posicion == 1){ // Si la posicion es izquierda
               objeto.naveNodrisa.gdireccion = objeto.naveNodrisa.getX;
               objeto.naveNodrisa.sdireccion = objeto.naveNodrisa.setX;
            } else if(posicion == -1){ // Si la posicion es derecha
               objeto.naveNodrisa.gdireccion = objeto.naveNodrisa.getY;
               objeto.naveNodrisa.sdireccion = objeto.naveNodrisa.setY;
            }
         },1000);
   ...
   this.desplazamiento = function(e) {
      ...
      if (e.keyCode == '38'|| e.keyCode == '40') {
         //arriba
         objeto.naveConTeclas.sdireccion=objeto.naveNodrisa.setY;
         objeto.naveConTeclas.gdireccion=objeto.naveNodrisa.getY; 


         if (e.keyCode == '38'){ //Arriba
            objeto.naveConTeclas.setVelocidad(-3);
         ...
         objeto.naveConTeclas.sdireccion=objeto.naveNodrisa.setX;
         objeto.naveConTeclas.gdireccion=objeto.naveNodrisa.getX;
      ...
      if (objeto.estado == CREACION) {
         ...
         if(objeto.estado==PRECARGA){
            ...
            if(imagenesCargadas==true){ // si todas las imagenes estan cargadas
               ...
               objeto.naveConTeclas = new NaveEspacial(objeto.random(0,objeto.canvas.width-objeto.imagenes[3].width),objeto.random(0,objeto.canvas.height-objeto.imagenes[3].height),objeto.imagenes[3]); // crea la nave
               objeto.naveConTeclas.gdireccion=objeto.naveConTeclas.getX; // establece la funcion que devuelve la posicion en x
               objeto.naveConTeclas.sdireccion=objeto.naveConTeclas.setX; // establece la funcion que modifica la posicion en x
